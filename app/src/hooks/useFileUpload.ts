...
import { isAndroidPlatform, showFileDialogFallback, pickWithFallback, debugLog } from '../utils';
...
export function useFileUpload(activeFolderId: number | null, store: Store | null) {
    ...
    const processItem = async (item: QueueItem) => {
        ...
        try {
            // #region agent log
            debugLog('useFileUpload.ts:processItem', 'upload starting', {
              pathPrefix: item.path?.slice(0, 80),
              hasContentUri: item.path?.includes('content://'),
              isAndroidPlatform,
            ...
            // #region agent log
            debugLog('useFileUpload.ts:processItem', 'upload failed', {

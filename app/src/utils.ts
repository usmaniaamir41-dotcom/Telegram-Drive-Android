...
/** Resolve platform flags synchronously (avoids usePlatform's false initial state). */
export function getPlatformFlags() {
  const ua = typeof navigator !== 'undefined' ? navigator.userAgent.toLowerCase() : '';
  const uaAndroid = ua.includes('android');
  const uaIos = ua.includes('iphone') || ua.includes('ipad');
  try {
    const os = osType();
    const isAndroid = os === 'android' || uaAndroid;
    const isIos = os === 'ios' || uaIos;
    return { os, isAndroid, isIos, isMobile: isAndroid || isIos };
  } catch {
    const isAndroid = uaAndroid;
    const isMobile = isAndroid || uaIos;
    return { os: 'unknown', isAndroid, isIos: uaIos, isMobile };
  }
}

export const isAndroidPlatform = getPlatformFlags().isAndroid;

/** Touch-first narrow layout (phones / small Android tablets). */
export function isLikelyPhoneUI(): boolean {
  if (typeof window === 'undefined') return false;
  const flags = getPlatformFlags();
  if (flags.isMobile) return true;
  const coarse = window.matchMedia('(pointer: coarse)').matches;
  const narrow = window.innerWidth < 900;
  return coarse && narrow;
}

/** Android WebView often reports env(safe-area-inset-*) as 0px — set minimum insets. */
export function initPlatformChrome() {
  if (typeof document === 'undefined') return;
  const flags = getPlatformFlags();
  const root = document.documentElement;
  if (flags.isAndroid) {
    root.classList.add('platform-android');
    root.style.setProperty('--safe-area-min-top', '32px');
    root.style.setProperty('--safe-area-min-bottom', '0px');
  }
  if (flags.isIos) {
...
export function debugLog(
  ...
  fetch('http://127.0.0.1:7870/ingest/91095e8b-6fd6-4ecc-a78f-9532eaf5dbcb', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'X-Debug-Session-Id': '960e51' },
    body: payload,
  }).catch(() => {});
  invoke('cmd_debug_session_log', { payload }).catch(() => {});
  invoke('cmd_log', { message: `[DEBUG-960e51] ${payload}` }).catch(() => {});
}
// #endregion

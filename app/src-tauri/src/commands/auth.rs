...
pub async fn ensure_client_initialized(
    ...
    connection_params.device_model = "Android".into();
    connection_params.system_version = "14.0".into();
    connection_params.app_version = "10.14.5".into();
    connection_params.system_lang_code = "en-US".into();
    connection_params.lang_code = "en".into();
...
pub async fn cmd_auth_request_code(
    ...
    *state.api_id.lock().await = Some(api_id);
    ...
    let api_hash_trimmed = api_hash.trim().to_string();
    
    // Retry up to 2 times for AUTH_RESTART or 500
    for i in 1..=2 {
        match client_handle.request_login_code(&phone, &api_hash_trimmed).await {
            Ok(token) => {
